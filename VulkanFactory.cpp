    ...
    : public IVulkanFactory
{
    ...
    uint32_t GetMemoryIndex(uint32_t type_bits, VkMemoryPropertyFlags memory_property)
    {
...
public:
    ...
    Buffer CreateBuffer(VkBufferUsageFlags usage, VkMemoryPropertyFlags memory_property, VkDeviceSize size, void* data) override
    {
        Buffer buffer = {};
        buffer.size = size;
        ...
        memAllocInfo.allocationSize = memory_requiments.size;
        memAllocInfo.memoryTypeIndex = GetMemoryIndex(memory_requiments.memoryTypeBits, memory_property);
        VkResultSuccess(m_device_functions.vkAllocateMemory(m_device, &memAllocInfo, nullptr, &buffer.memory));
